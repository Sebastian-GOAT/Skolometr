...
export default function LoginPage() {
    ...
    useEffect(() => {
        document.title = 'Přihlášení | Školometr';
    ...
    async function handleSubmit(e: FormEvent): Promise<void> {
        ...
        async function signIn(): Promise<User | null> {
            try {
                const credentials = await signInWithEmailAndPassword(auth, email as string, password as string);
            ...
            <main className='py-16 w-full h-fit min-h-[calc(100vh_-_60px)] flex justify-center items-center'>
                <div className='p-4 w-[250px] h-fit border-1 rounded-2xl'>
                    <form onSubmit={handleSubmit} className='w-full h-full gap-4 flex flex-col items-center'>
                        ...
                        <h1 className='mb-4 text-[1.4rem] font-medium'>Vítej zpět!</h1>
                        <input
                            type="email"
                            ref={emailRef}
                            placeholder='E-Mail...'
                        ...
                        <p className='text-[0.75rem] gap-1 flex'>
                            Ještě nemáš účet?
                            <Link to='/auth/signup' className='text-[var(--accent)]'>Zaregistruj se</Link>
                        </p>
                        <button className='mt-2 p-2 hover:text-[var(--accent)] outline-0 cursor-pointer transition-colors duration-200'>Přihlásit se</button>
